...
async function resolveUserAccessScope(supabaseAdmin, userId) {
  ...
  if (mappingError || !Array.isArray(mappingRows)) {
    return {
      appIds: [],
      appOrderById: new Map(),
      roleIdsByApp: new Map(),
    };
  }
  ...
  activeMappings.forEach((mapping) => {
    ...
    if (!roleIdsByApp.has(appId)) {
      roleIdsByApp.set(appId, new Set());
    }
...
async function loadAssignedCardsFromDatabase() {
  try {
    ...
    if (!Array.isArray(accessScope.appIds) || accessScope.appIds.length === 0) {
      return [];
    }
    ...
    for (const appId of accessScope.appIds) {
      ...
      if (!(roleSet instanceof Set) || roleSet.size === 0) {
        continue;
      }
      ...
      if (groupError || !Array.isArray(groupRows) || groupRows.length === 0) {
        continue;
      }
      ...
      if (cardError || !Array.isArray(cardRows) || cardRows.length === 0) {
        continue;
      }
      ...
      if (cardRoleError || !Array.isArray(cardRoleRows)) {
        continue;
      }
      ...
      cards.forEach((card) => {
        if (!visibleCardIds.has(card.cardId)) {
          return;
        }
...
async function loadAssignedAppsFromDatabase() {
  try {
    const cookieStore = await cookies();
    const accessToken = cookieStore.get("sb-access-token")?.value;
      ...
      .map((appId, index) => {
        ...
        return {
          ...
          path: normalizeRoutePath(
            readText(appRecord, ["route_path", "route", "path", "href", "module_path"], "#"),
          ),
          appId,
          ...
          groupIcon: "bi-collection",
          order: Number(
            appRecord?.display_order ?? appRecord?.app_order ?? appRecord?.sort_order ?? appRecord?.order_no ?? index + 1,
          ),
        };
      })
