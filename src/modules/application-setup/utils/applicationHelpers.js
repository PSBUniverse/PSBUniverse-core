...
export async function executeBatchSave(pendingBatch, orderedApplications) {
  ...
  for (const entry of pendingBatch.appCreates || []) {
    const res = await requestJson("/api/application-setup/applications", {
  ...
  for (const [appId, updates] of Object.entries(pendingBatch.appUpdates || {})) {
    ...
    await requestJson(`/api/application-setup/applications/${encodeURIComponent(String(resolved))}`, {
  ...
  for (const entry of pendingBatch.roleCreates || []) {
    ...
    await requestJson("/api/application-setup/roles", {
  ...
  for (const [roleId, updates] of Object.entries(pendingBatch.roleUpdates || {})) {
    ...
    await requestJson(`/api/application-setup/roles/${encodeURIComponent(String(roleId))}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updates),
  ...
  for (const appId of pendingBatch.appDeactivations || []) {
    ...
    await requestJson(`/api/application-setup/applications/${encodeURIComponent(String(appId))}`, {
      method: "DELETE",
  ...
  if (orderedPersistedAppIds.length > 0) {
    await requestJson("/api/application-setup/order", {
      method: "POST", headers: { "Content-Type": "application/json" },
