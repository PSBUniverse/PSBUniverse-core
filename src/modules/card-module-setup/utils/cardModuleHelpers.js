...
export async function executeBatchSave(pendingBatch, appGroups, allCards, persistedCardOrderSignatures) {
  ...
  for (const entry of pendingBatch.groupCreates || []) {
    const res = await requestJson("/api/card-module-setup/card-groups", {
  ...
  for (const [gid, updates] of Object.entries(pendingBatch.groupUpdates || {})) {
    ...
    await requestJson(`/api/card-module-setup/card-groups/${encodeURIComponent(String(resolved))}`, {
  ...
  for (const entry of pendingBatch.cardCreates || []) {
    ...
    await requestJson("/api/card-module-setup/cards", {
  ...
  for (const [cid, updates] of Object.entries(pendingBatch.cardUpdates || {})) {
    ...
    await requestJson(`/api/card-module-setup/cards/${encodeURIComponent(String(cid))}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updates),
  ...
  for (const cid of pendingBatch.cardDeactivations || []) {
    ...
    await requestJson(`/api/card-module-setup/cards/${encodeURIComponent(String(cid))}`, { method: "DELETE" }, "Failed to deactivate card.");
  }

  for (const gid of pendingBatch.groupDeactivations || []) {
    if (isTempGroupId(gid)) continue;
    await requestJson(`/api/card-module-setup/card-groups/${encodeURIComponent(String(gid))}`, { method: "DELETE" }, "Failed to deactivate card group.");
  }
  ...
  if (orderedPersistedGroupIds.length > 0) {
    await requestJson("/api/card-module-setup/card-groups-order", {
      method: "POST", headers: { "Content-Type": "application/json" },
  ...
  for (const group of appGroups) {
    ...
    if (orderedCardIds.length > 0) {
      await requestJson("/api/card-module-setup/cards-order", {
        method: "POST", headers: { "Content-Type": "application/json" },
