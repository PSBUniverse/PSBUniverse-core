...
export async function executeBatchSave(pendingBatch, refreshUsers) {
  ...
  for (const entry of pendingBatch.creates || []) {
    const resp = await fetch("/api/admin/user-master-setup/users",
    ...
    for (const access of entry?.accessRows || []) {
      const ar = await fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(createdId)}/access`,
  ...
  for (const [rawId, payload] of Object.entries(pendingBatch.updates || {})) {
    ...
    const resp = await fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(resolvedId)}`,
  ...
  for (const [rawId, rows] of Object.entries(pendingBatch.accessDeletes || {})) {
    ...
    for (const row of rows || []) {
      const resp = await fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(resolvedId)}/access`,
...
export async function executeDeactivateUser(userId) {
  const resp = await fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(userId)}`, { method: "DELETE" });
...
export async function fetchLookups() {
  const resp = await fetch("/api/admin/user-master-setup/lookups", { method: "GET", cache: "no-store" });
  const payload = await parseJsonResponse(resp, "Failed to load lookups.");
  return payload?.lookups || EMPTY_LOOKUPS;
}

export async function fetchUsers() {
  const resp = await fetch("/api/admin/user-master-setup/users?limit=200", { method: "GET", cache: "no-store" });
...
export async function fetchUserDetail(userId) {
  const [detailResp, accessResp] = await Promise.all([
    fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(userId)}`, { method: "GET", cache: "no-store" }),
    fetch(`/api/admin/user-master-setup/users/${encodeURIComponent(userId)}/access`, { method: "GET", cache: "no-store" }),
  ]);
